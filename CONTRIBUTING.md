...
If you have a question rather than an issue, please post it to the [NexusForge Stack 
Overflow tag](https://stackoverflow.com/questions/tagged/nexusforge). For non-security related bugs please log a new issue:

1. Search the [issue tracker](https://github.com/NexusForgeIO/NexusForge/issues) for similar issues.
2. Specify the **version** of `NexusForge.Core` package in which the bug was occurred.
3. Specify the **storage** package (e.g. `NexusForge.SqlServer`) you are using and its exact version.
4. Specify the **configuration** logic for NexusForge.
...
* Include log messages, written by NexusForge when a problem occurred. Don't forget to tell your logger to dump all the exception details.
